...
const EditSubtopicModal = ({ subtopic, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    await onSave(subtopic._id, subtopicName.trim(), description.trim());
    setLoading(false);
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 rounded-2xl w-full max-w-md border border-slate-700">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-700">
          <div>
            <h2 className="text-xl font-semibold text-white">Edit Subtopic</h2>
            <p className="text-sm text-slate-400 mt-1">
              Subject: <span className="text-primary-400 font-medium">{subtopic.category}</span>
            </p>
          </div>
          <button
            onClick={onClose}
